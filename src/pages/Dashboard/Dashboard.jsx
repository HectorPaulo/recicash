...
const Dashboard = () => {
  ...
  const mensajeDormir =
    horaActual >= 19 || horaActual <= 6 ? " ¡Es hora de descansar!" : "";
    ...
    if (isAdmin) {
      Promise.all([
        ...
        .then(([clientesRes, empresasRes, cuponesRes]) => {
          // Acepta array directo o {clientes: [...]}
          ...
          const cuponesData = Array.isArray(cuponesRes.data)
            ? cuponesRes.data
            : cuponesRes.data.cupones || [];
      ...
      axios
        .get(
          `${import.meta.env.VITE_API_URL}/cliente/${currentUser.id}/cupones`
        )
        .then((res) => {
          ...
          const puntosAcumulados = cuponesCliente.reduce(
            (acc, cupon) => acc + (cupon.precio || 0),
            0
          );
      ...
      // Corregimos la lógica para obtener los cupones de una empresa
      // El problema puede ser que estamos usando mal el endpoint o la estructura del ID

      // Intentamos obtener el ID correcto para la empresa
      const empresaId = currentUser?.id || currentUser?.user_id?.id;
      console.log("ID de empresa utilizado:", empresaId);

      axios
        .get(`${import.meta.env.VITE_API_URL}/cupon/empresa/${empresaId}`)
        .then((res) => {
          console.log("Respuesta de cupones para empresa:", res.data);

          // Manejamos diferentes formatos de respuesta posibles
          let cuponesData = [];
          if (Array.isArray(res.data)) {
            cuponesData = res.data;
          } else if (res.data && typeof res.data === "object") {
            // Si es un objeto, intentamos extraer el array de cupones
            if (Array.isArray(res.data.cupones)) {
              cuponesData = res.data.cupones;
            } else {
              // Si no tiene una propiedad cupones, podría ser el array directamente
              cuponesData = Object.values(res.data).filter(
                (item) => typeof item === "object"
              );
            }
          }

          console.log("Cupones procesados:", cuponesData);
          setCupones(cuponesData);
        ...
        .catch((error) => {
          console.error("Error al obtener cupones de empresa:", error);
          setCupones([]);
        })
        .finally(() => setLoading(false));
    }
  ...
  if (isAdmin) {
    ...
    cupones.forEach((cupon) => {
      const fecha = new Date(
        cupon.createdAt || cupon.fechaCreacion || cupon.fechaExpiracion
      );
      if (!isNaN(fecha)) {
      ...
      datasets: [
        {
          ...
          backgroundColor: ["rgb(21,94,35, 0.7)", "rgba(20,147,30,0.7)"],
          borderColor: [
            "rgba(21,94,35, 1)",
            "rgba(16, 185, 129, 1)",
          ],
        ...
        <span
          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
            isAdmin
              ...
              : isEmpresa
              ? "bg-purple-100 text-green-700-800"
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">
                    Bienvenido,{" "}
                    <span className="text-green-800">{nombre}</span>
                    {mensajeDormir && (
                    ...
                    {cupones.length === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-600">
                          No has participado en ningún cupón aún
                        </p>
                        <Link
                          to="/cupones"
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    {stats.activos === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-600">
                          No tienes cupones activos actualmente
                        </p>
                        <Link
                          to="/cupones/nuevo"
                          className="mt-4 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700"
