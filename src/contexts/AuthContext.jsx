/* eslint-disable no-unused-vars */
    ...
    export function AuthProvider({ children }) {
      ...
      const crearCliente = async (token, user_id) => {
      ...
      const register = async (nombre, email, password, telefono) => {
        ...
        try {
          ...
          if (data.token && data.id) {
            ...
            const clienteExistente = await getClienteByEmail(data.token, email);
            if (!clienteExistente) {
              await crearCliente(data.token, data.id);
            }
          ...
          return { error: { message: "No se recibió token o id del servidor" } };
        } catch (error) {
          return { error };
        }
      };
